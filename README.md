...
It holds the cryptographic timestamps for one frozen ledger snapshot — **not** the ledger's contents. The forecasts themselves are members-only ([sigmafutures.group](https://sigmafutures.group)). What this repository proves to anyone, without their having to trust SIGMA, is that this exact ledger existed and was anchored to the Bitcoin blockchain on the snapshot date below — so no forecast in it can have been added, altered, or back-dated afterwards.
...
|------|---------|-----------------|
| `SIGMA-Forecast-Ledger-PUBLIC.csv` | `106eeeae204a93b992b2ea276028a2eafadfd5a360a9a439958c003a537938fd` | `SIGMA-Forecast-Ledger-PUBLIC.csv.ots` |
| `SIGMA-Forecast-Ledger-PUBLIC.xlsx` | `2dccd811eaea9c4ff05d2596360410c35a2330778d10a5cb19d8bdadd0c23be2` | `SIGMA-Forecast-Ledger-PUBLIC.xlsx.ots` |
...
- *Command line:* `pip install opentimestamps-client`, then `ots verify SIGMA-Forecast-Ledger-PUBLIC.csv.ots` (with the original file in the same folder).
...
```
shasum -a 256 SIGMA-Forecast-Ledger-PUBLIC.csv               # macOS / Linux
certutil -hashfile SIGMA-Forecast-Ledger-PUBLIC.csv SHA256   # Windows
```
